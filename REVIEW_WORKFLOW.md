...
- EAG에서는 **fall–rise가 4번 반복**되어 knee-pair 8개가 나온다
...
| `detect_eag_edges_protocol()` | 전역 검출 + **anchor별 국소 보강**(전역 문턱에 묻힌 약한 반응 회수) |
...
- **부하/이탈 방향이 같으면 한쪽이 오염된 것**이므로 자동으로 한쪽만 채택한다.
  ...
  (채널·피험자에 따라 극성이 뒤집힐 수 있어 하드코딩하지 않는다).
...
- **회색 점선 edge** = 노이즈 후보 (anchor에서 먼 것). 표의 `판정` 열에도 `노이즈`로 표시
...
- **휠**=가로축 확대/축소(커서 기준) · **Shift+드래그**(또는 휠버튼 드래그)=좌우 이동 ·
  **f키/전체보기**=원래대로. 확대하면 y축도 그 구간에 맞춰 재조정되고 snap 범위도 좁아져 정밀해진다
...
| 항목 | 값 |
|---|---|
| 전체 | 세션 425개 · 채널 3,339개 |
| cycle 4회 검출 | **399 / 425 세션** |
| 프로토콜 통과 채널 | **3,089 / 3,339 (93%)** |
| `high` (사람이 봐야 함) | **239 채널 / 77 세션** |
| `low` (한쪽만 자동채택, 후순위) | 2,247 채널 |
| 확정 offset / edge | 2 세션 / 1 채널 |

`high` 사유 내역: `측정 불가 cycle 1개` 99 · `cycle 3회` 67 · `cycle 1회` 24 · `cycle 2회` 12 · `cycle 5회` 8 …
→ 절반가량이 **cycle이 4회로 안 잡히는 16개 세션**에서 나온다. 이 세션들을 먼저 열어보면 효율적이다.
