...
- **웹 브라우저** + **code-server 접속 비밀번호** (교수님께 받으세요)
- 그게 전부입니다. 파이썬 설치, 데이터 복사, git clone **모두 불필요** — 데이터·앱·결과가 서버에 이미 있습니다.

> ⚠️ **비밀번호는 연구실 공용입니다.** 개인 계정이 아니라 **모두가 같은 비밀번호를 쓰는 구조**라,
> 로그인하면 서버 전체(다른 프로젝트 폴더·터미널 포함)에 접근됩니다.
> - **외부에 공유하지 마세요.**
> - 이 작업에 필요한 건 아래 두 GUI 주소뿐입니다. **다른 폴더·저장소는 건드리지 마세요.**

## 0-1. 처음 접속 (한 번만)

1. 브라우저에서 **`http://medibb.synology.me:18440/`** 접속 → 받은 비밀번호로 로그인
2. 로그인 상태가 유지되면 아래 2번의 GUI 주소가 바로 열립니다
   (로그인이 안 된 상태로 GUI 주소를 열면 로그인 화면이 뜹니다 — 로그인 후 다시 여세요)
...
0-1에서 로그인한 **같은 브라우저**로 아래 주소를 엽니다 (**끝 슬래시 `/` 필수**).
즐겨찾기에 넣어두면 매번 이 두 개만 열면 됩니다:
...
- 비밀번호가 공용이므로 **`/workspace` 아래 다른 프로젝트 폴더는 열지도, 고치지도 마세요.**
  이 작업에 필요한 건 `research/EAGstatic` 하나뿐입니다.
- 터미널에서 `git` 명령은 **`result/` 커밋(4장)** 외에는 쓰지 마세요. 브랜치 전환(`git checkout`)은
  진행 중인 확정값 파일을 되돌릴 수 있으니 **하지 마세요.**
