...
class EAGAnalyzer:
    ...
    def generate_report(self, output_dir: Optional[str] = None):
        ...
        if output_dir is None:
            # result 폴더 내에 피험자/세션 구조로 저장
            result_base = Path(get_result_dir())

            # 파일 경로에서 피험자와 세션 정보 추출
            filepath = Path(self.filepath)
            parts = filepath.parts

            # data 폴더 이후의 경로 구조 찾기 (피험자/세션)
            subject_name = None
            session_name = None
            ...
            for i, part in enumerate(parts):
                if part == 'data' and i + 1 < len(parts):
                    ...
                    if i + 2 < len(parts):
                        session_name = parts[i + 2]
                    break

            # 출력 경로 구성
            if subject_name and session_name:
                output_dir = result_base / subject_name / session_name
            elif subject_name:
...
def list_subjects(data_dir: str) -> List[str]:
    """data 폴더 내 피험자 폴더 목록 반환"""
...
def interactive_select_data() -> Optional[str]:
    ...
    if not sessions:
        print(f"{subject_name}에 세션 데이터가 없습니다.")
        return None

    print("\n" + "="*60)
    print(f"=== 세션 선택 ({subject_name}) ===")
    print("="*60)
